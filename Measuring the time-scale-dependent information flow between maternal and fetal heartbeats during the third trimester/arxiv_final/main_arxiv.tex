...
\author{Nicolas B. Garnier$^{1}$*, Sol Molinet$^{2}$, Marta Antonelli$^{2,3}$, Silvia Lobmaier$^{2}$, Martin Frasch$^{4,5}$*\\
\\
\small $^{1}$CNRS, ENS de Lyon, LPENSL, UMR5672, 69342, Lyon cedex 07, France\\
\small $^{2}$Technical University of Munich; Institute for Advanced Study, Garching, Germany\\
\small $^{3}$Instituto de Biología Celular y Neurociencia "Prof. Eduardo De Robertis", Facultad de Medicina, Universidad de Buenos Aires, Argentina\\
...
\\
\small *Correspondence: nicolas.garnier@ens-lyon.fr, mfrasch@uw.edu}
...
\noindent\textbf{Keywords:} multiscale analysis ; information theory ; high-order statistics ; fetal heart rate ; causality
...
\begin{itemize}
    \item 20 ER features: max/mean $\times$ fetus/mother $\times$ \{full, fHR\_accel, fHR\_decel, mHR\_accel, mHR\_decel\}
    \item 20 SE features: Same structure as ER
    \item 12 TE features: max/mean $\times$ \{fHR/mHR conditioning\} $\times$ \{all, accel, decel\}
...
Data structure: 480 observations (120 patients $\times$ 4 measurements each: mHR\_accel, mHR\_decel, fHR\_accel, fHR\_decel). Random intercept accounts for patient-level correlation.
...
Mixed linear model analysis revealed a significant main effect of event type ($\beta$ = -0.061, SE = 0.0028, p < 0.001) and HR\_Source $\times$ Event\_Type interaction ($\beta$ = 0.028, SE = 0.0028, p < 0.001).
...
\begin{itemize}
    ...
    \item Sex $\times$ Stress interaction: $\beta$ = -0.0001, p = 0.985
\end{itemize}

Acceleration/deceleration asymmetry represents a universal biological phenomenon, consistent across demographic groups in this third-trimester cohort. Full statistical results are presented in % COMMENTED: undefined reference \ref{\1mlm_acc_dec_complete}.
...
Mixed linear model analysis of entropy rate with conditioning framework revealed both univariate signal properties and bivariate maternal-fetal coupling (Figures ~\ref{fig:complexities_boxplots_no_conditioning},~\ref{fig:complexities_boxplots}, % COMMENTED: undefined reference \ref{\1mlm_er_complete} ).

\medskip

\textbf{Conditioning Effects: Coupling Strength Quantification}

Mixed linear model analysis revealed that maternal deceleration conditioning showed the strongest coupling effect ($\beta$ = -0.123, p = 0.012), representing approximately 60\% coupling strength relative to the no-conditioning baseline ($\beta$ = +0.206, p < 0.001). This coupling strength quantifies how much of the available entropy variation is constrained by maternal bradycardic states (see Supplementary Box % COMMENTED: undefined reference \ref{\1coupling_box} for detailed calculation methodology). Importantly, this profound coupling effect is stress-invariant (p = 0.128 for stress effect on conditioned entropy), indicating a fundamental physiological coordination mechanism present across all mother-fetus pairs regardless of maternal stress status.
...
Maternal deceleration events exhibit 1.5$\times$ stronger coupling than fetal events and 3.8$\times$ stronger coupling than maternal accelerations, demonstrating a profound asymmetry in maternal-fetal physiological interdependence. Importantly, this asymmetric pattern is conserved across stressed and control groups, suggesting that it reflects a fundamental maternal-fetal communication architecture rather than a stress-specific adaptation.
...
\begin{itemize}
    ...
    \item Sex $\times$ Stress: $\beta$ = +0.108, p = 0.223 (ns)
\end{itemize}

Of note, while transfer entropy showed significant stress correlations (r = 0.21-0.31 with cortisol, see Table~\ref{tab:te_correlations} ), conditioned entropy rate showed no stress effects (p = 0.128). This differential stress sensitivity between TE (stress-modulated) and conditioned entropy (stress-invariant) is explored in detail in Section 3.3.2. 
...
Two SE associations with neurodevelopmental outcomes reached nominal significance (p < 0.05, uncorrected; exploratory per Section 3.3.3 framework; Table~\ref{tab:erse_correlations}, Fig.~% COMMENTED: undefined reference \ref{\1heatmap_all}).

\textbf{Exploratory interpretation}: Sample entropy during fetal HR accelerations showed tentative positive associations with language receptive scores (higher entropy → better receptive language).

No significant associations were observed between ER/SE features and maternal stress measures (cortisol, PSS, PDQ), tentatively suggesting distinct pathways whereby TE may capture stress physiology while ER/SE relate to neurodevelopment.
...
Sex stratification revealed differential ER/SE association patterns (Fig. % COMMENTED: undefined reference \ref{\1se_er_sex_heatmap}):

\textbf{Female fetuses} (n=71): In addition to the 16 TE correlations (Section 3.3.3), females showed:

\begin{itemize}
    \item ER $\times$ Motor Gross (4 negative correlations, r = -0.37 to -0.61): Fetal ER during various conditioning types showed negative associations with motor gross skills
    \item SE $\times$ Cognitive/Language (3 positive correlations):
    \begin{itemize}
        \item SE mother full $\times$ Cognitive: r = +0.44, p = 0.011, n = 33
        \item SE fetus (fHR accel) $\times$ Lang Receptive: r = +0.41, p = 0.024, n = 30
        \item SE mother (fHR accel) $\times$ Lang Receptive: r = +0.41, p = 0.024, n = 30
    \end{itemize}
\end{itemize}
\textbf{Male fetuses} (n=49): Only the previously noted ER $\times$ Motor Composite correlation (r = +0.39, p = 0.035).


\textbf{Sex $\times$ Stress stratification} revealed additional complexity (Fig. % COMMENTED: undefined reference \ref{\1se_er_sex_stress_heatmap}):

\textbf{Male-Control}: Strong positive ER/SE $\times$ Motor associations dominated (8 ER correlations, 2 SE correlations with r = +0.49 to +0.72), with the strongest being SE fetus full $\times$ Motor Fine (r = +0.72, p < 0.001, $n=20$).

\textbf{Female-Control}: Positive ER $\times$ Cognitive/PDQ associations (5 correlations, r = +0.38 to +0.65) rather than motor associations.
...
Next, we apply mixed linear model analysis of transfer entropy values across conditioning types. This approach revealed significant effects of conditioning, TE metric type, and demographic interactions (see % COMMENTED: undefined reference \ref{\1te_mlm_complete} for details).
...
\begin{itemize}
    \item \textbf{TE metric $\times$ Conditioning interaction}:  $\beta$ = +0.064, SE = 0.010, p < 0.001***
\end{itemize}
...
\item \textbf{Sex $\times$ Stress interaction}: $\beta$ = -0.042, SE = 0.016, p = 0.009**\\
...
\item \textbf{Sex $\times$ Stress $\times$ Conditioning interaction}: $\beta$ = +0.037, SE = 0.015, p = 0.014*\\
...
\textbf{Interpretive Framework for Correlation Analyses:} All correlation analyses reported in this and subsequent sections (3.3.3-3.3.5) are exploratory. Of 144 correlation tests performed across all entropy features and outcomes, seven uncorrected associations ($p < 0.05$) were identified, matching the expected false positive rate of $\sim 7.2$ at $\alpha$ = 0.05 (Table~\ref{tab:te_correlations} and Fig.~% COMMENTED: undefined reference \ref{\1heatmap_all}). Critically, none of these associations survived false discovery rate (FDR) correction using the Benjamini-Hochberg procedure (all q-values > 0.40). Individual p-values are reported to facilitate hypothesis generation for future confirmatory studies, but all findings require independent replication in adequately powered samples before biological interpretation.
...
\textbf{Exploratory interpretation} (see interpretive framework above): All uncorrected significant TE-cortisol associations were positive (r = 0.22-0.31), tentatively suggesting that stronger directional coupling between maternal and fetal heart rates may be associated with higher chronic stress. Maximum TE values showed stronger associations than mean TE values.
...
\textbf{Exploratory interpretation} (see Section 3.3.3 interpretive framework): Higher TE during fetal accelerations was tentatively associated with lower expressive language skills. This was the only TE-neurodevelopmental association to reach nominal significance among 30 tests performed. No significant TE associations were observed with cognitive or motor outcomes, or with psychological stress measures (PSS, PDQ).



\subsubsection{Sex-stratified TE analysis (Exploratory)}

Sex-stratified analysis revealed striking differences in TE-outcome associations, consistent with the robust Sex $\times$ Stress $\times$ TE interaction from MLM analysis:

\textbf{Female fetuses }(n=71) exhibited 16 significant correlations (p < 0.05, uncorrected; exploratory per Section 3.3.3 framework).
...
\textbf{Male fetuses} (n=49) exhibited only one significant correlation (Entropy Rate fetus mHR decel $\times$ Motor Composite: r = +0.387, p = 0.035, n = 30; exploratory). No significant TE-cortisol or TE-cognitive performance correlations were seen in males.

\textbf{Exploratory interpretation}: Female-specific maternal-fetal coupling mechanisms may underlie the robust Sex $\times$ Stress $\times$ TE interaction. In females, higher TE tentatively associates with both higher maternal stress (cortisol) and lower cognitive scores, suggesting that increased directional coupling under stress may reflect compensatory or maladaptive physiological responses. The absence of TE correlations in males tentatively suggests sex-differentiated autonomic regulation pathways.

\subsubsection{Sex $\times$ Stress Interaction Patterns (Exploratory)}

Further stratification by sex and stress status revealed complex interaction patterns (Figure % COMMENTED: undefined reference \ref{\1se_er_sex_stress_heatmap}):

Male-Control fetuses (n=30) exhibited 16 significant correlations (p < 0.05, uncorrected; exploratory):

\begin{itemize}
    \item Strong positive ER/SE $\times$ Motor associations (r = +0.49 to +0.72)
    \item Positive Mean TE $\times$ PSS associations (r = +0.43 to +0.45)
    \item Positive Mean TE $\times$ Cognitive (r = +0.48)
\end{itemize}

Male-Stressed fetuses (n=19) showed seven significant correlations (exploratory):

\begin{itemize}
    \item Pattern reversal: Negative Max TE $\times$ PSS (r = -0.49, p = 0.032)
    \item Negative Mean TE $\times$ Language scores (r = -0.65 to -0.70)
    \item Motor associations absent
\end{itemize}

Female-Control fetuses (n=32) exhibited twelve significant correlations (exploratory):

\begin{itemize}
    \item Strongest TE-cortisol coupling: Max TE (mHR decel) r = +0.55, p = 0.006
    \item Very strong negative TE $\times$ Language: Max TE (fHR accel) $\times$ Lang Composite r = -0.86, p < 0.001
    \item Positive ER $\times$ Cognitive/PDQ (r = +0.38 to +0.65)
\end{itemize}

Female-Stressed fetuses (n=39) showed seven significant correlations (exploratory):
...
\begin{itemize}
    ...
    \item Positive SE mother $\times$ Cognitive (r = +0.54, p = 0.014)
\end{itemize}

\textbf{Exploratory interpretation}: These sex $\times$ stress patterns tentatively suggest that in:
...
These exploratory stratified findings provide potential mechanistic context for the robust Sex $\times$ Stress $\times$ TE interaction.
...
In summary, the univariate correlation analyses and the MLM analyses with reduced feature sets provide the most reliable findings for this sample size. Multivariate models would require sample sizes of n > 500 (approximately 10 $\times$ 52 features) for valid inference.
...
Second, we discovered differential stress sensitivity in these coupling pathways (Section 3.3.2). Temporal information transfer shows stress-related modulation and exploratory associations with maternal cortisol, while state-dependent coupling remains stress-invariant—a fundamental physiological coordination that persists regardless of maternal stress status.
...
Critically, we extend the FSI framework by demonstrating that coupling operates through dual pathways (Section 3.3.2). The stress-invariant state-dependent coupling (conditioned entropy) represents the fundamental physiological coordination mechanism existing across all mother-fetus pairs, while the stress-sensitive temporal coupling (transfer entropy) captures the "over-sensitization" hypothesized in our previous work—where maternal stress alters the fetal autonomic system's temporal response dynamics.
...
As demonstrated in Section 3.3.2, this differential stress sensitivity between temporal coupling (TE) and state-dependent coupling (conditioned entropy) may reflect different communication pathways. The stress-invariant state-dependent coupling could represent direct mechanical or circulatory coupling, while the stress-modulated temporal coupling might involve more complex autonomic and hormonal pathways susceptible to maternal stress effects.
...
Our MLM analysis revealed that mHR decelerations exert substantially stronger influence on fHR complexity than any other physiological state. The 60\% coupling strength (see Supplementary Box % COMMENTED: undefined reference \ref{\1coupling_box} for calculation details) quantifies how strongly maternal bradycardic state constrains fHR complexity relative to the available entropy variation—representing the ratio of the maternal deceleration effect ($\beta$ = -0.123) to the no-conditioning baseline ($\beta$ = +0.206). This normalized measure of coupling intensity establishes maternal decelerations as the strongest coupling condition detected in our analysis and facilitates comparison across different physiological states.

Critically, this represents a fundamental coupling mechanism conserved across all pregnancies—both stressed and control groups exhibit this profound entropy reduction during maternal decelerations. This stress invariance is essential for interpreting the findings in the context of past studies.
...
This apparent contradiction resolves when considering that FSI and conditioned entropy capture different coupling aspects. The entropy reduction (60\%) and the fHR response magnitude (FSI) capture complementary but distinct physiological phenomena:
...
\begin{figure}[htb]
\centering
\includegraphics[width=0.9\linewidth]{correlation_heatmaps_exploratory.png}
\caption{Exploratory correlation heatmap showing all entropy rate and sample entropy associations with neurodevelopmental outcomes.}
\label{fig:heatmap_all}
\end{figure}

\begin{figure}[htb]
\centering
\includegraphics[width=0.9\linewidth]{correlation_heatmaps_sex_stratified.png}
\caption{Sex-stratified correlation heatmaps for entropy rate and sample entropy associations with neurodevelopmental outcomes.}
\label{fig:se_er_sex_heatmap}
\end{figure}

\begin{figure}[htb]
\centering
\includegraphics[width=0.9\linewidth]{correlation_heatmaps_sex_stress_stratified.png}
\caption{Sex and stress stratified correlation heatmaps for entropy rate and sample entropy associations with neurodevelopmental outcomes.}
