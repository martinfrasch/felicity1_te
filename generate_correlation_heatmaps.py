#!/usr/bin/env python3
"""
Generate correlation heatmaps for exploratory findings
Shows ER/SE/TE features vs clinical outcomes
Three panels: All participants, Control group, Stressed group
"""
...
print(f"Patients with stress info: {merged['stress'].notna().sum()}")
...
# All participants
corr_all, pval_all, n_all = compute_correlations(merged, all_features, outcomes)

# Control group (stress == 0)
control_data = merged[merged['stress'] == 0]
corr_control, pval_control, n_control = compute_correlations(control_data, all_features, outcomes)

# Stressed group (stress == 1)
stressed_data = merged[merged['stress'] == 1]
corr_stressed, pval_stressed, n_stressed = compute_correlations(stressed_data, all_features, outcomes)
...
datasets = [
    (corr_all, pval_all, n_all, 'All Participants'),
    (corr_control, pval_control, n_control, 'Control Group'),
    (corr_stressed, pval_stressed, n_stressed, 'Stressed Group')
]
...
plt.suptitle('Exploratory Correlation Analysis: Entropy Features vs Clinical Outcomes\n' +
             'NOTE: None of these correlations survived FDR correction (all q > 0.40)',
             fontsize=16, fontweight='bold', y=0.995)

plt.tight_layout()
output_file = Path(script_dir) / 'correlation_heatmaps_exploratory.png'
...
print("\n" + "="*80)
print("SIGNIFICANT CORRELATIONS (p < 0.05, uncorrected)")
print("="*80)

for group_name, corr, pval, n in [('All', corr_all, pval_all, n_all),
                                    ('Control', corr_control, pval_control, n_control),
                                    ('Stressed', corr_stressed, pval_stressed, n_stressed)]:
    print(f"\n{group_name} Group:")
    print("-" * 80)
    sig_count = 0
