#!/usr/bin/env python3
"""
Generate sex-stratified correlation heatmaps for exploratory findings
Shows ER/SE/TE features vs clinical outcomes
Two panels: Male participants, Female participants
"""
...
print(f"Patients with sex info: {merged['sex'].notna().sum()}")
print(f"  Male (sex=1): {(merged['sex'] == 1).sum()}")
print(f"  Female (sex=0): {(merged['sex'] == 0).sum()}")
...
# Compute correlations for sex-stratified groups
print("\nComputing sex-stratified correlations...")

# Male participants (sex == 1)
male_data = merged[merged['sex'] == 1]
corr_male, pval_male, n_male = compute_correlations(male_data, all_features, outcomes)

# Female participants (sex == 0)
female_data = merged[merged['sex'] == 0]
corr_female, pval_female, n_female = compute_correlations(female_data, all_features, outcomes)
...
# Create figure with two heatmaps (sex-stratified)
fig, axes = plt.subplots(1, 2, figsize=(18, 16))

datasets = [
    (corr_male, pval_male, n_male, 'Male Fetuses'),
    (corr_female, pval_female, n_female, 'Female Fetuses')
]
...
for ax, (corr, pval, n, title) in zip(axes, datasets):
    ...
    ax.set_xlabel('Clinical Outcomes', fontsize=12, fontweight='bold')
    if ax == axes[0]:
        ax.set_ylabel('Entropy Features', fontsize=12, fontweight='bold')

    # Rotate labels
    ax.set_xticklabels(ax.get_xticklabels(), rotation=45, ha='right', fontsize=9)
    ax.set_yticklabels(ax.get_yticklabels(), rotation=0, fontsize=8)

plt.suptitle('Sex-Stratified Correlation Analysis: Entropy Features vs Clinical Outcomes\n' +
             'NOTE: None of these correlations survived FDR correction (all q > 0.40)',
             fontsize=16, fontweight='bold', y=0.995)

plt.tight_layout()
output_file = Path(script_dir) / 'correlation_heatmaps_sex_stratified.png'
...
print("\n" + "="*80)
print("SEX-STRATIFIED SIGNIFICANT CORRELATIONS (p < 0.05, uncorrected)")
print("="*80)

for group_name, corr, pval, n in [('Male', corr_male, pval_male, n_male),
                                    ('Female', corr_female, pval_female, n_female)]:
    print(f"\n{group_name} Fetuses:")
    print("-" * 80)
    sig_count = 0
